mainApp.controller('cpsController', function ($scope, boundle, bundleRequest, $window, utilitiesFactory, httpFactory) {
	...
	$containerListIstanze = $("#containerListIstanze").DataTable({
		"scrollX"		: true,
		"order"			: [[ 1, "desc" ], [ 0, "desc" ]],
		"language"		: bIta.itaDatatable,
		"lengthMenu"	: [[10, 20], [10, 20]],
	...
	$("#closeModalFileCPS").off().on("click", function(){
	...
	$scope.chiudiListaIstanza = function(){
		$("#modalLIstaIstanze").modal("hide");
	}
	...
	$scope.uploadModel = function(model){
		model = $("#programmaUploadName").html();
		...
		renderTablePromise.success(function(response){
			if (response["RC"] == "0" || response["RC"]==0){
				...
				for (item in response){
					...
					var $openIstance = $("<img nomeCps='"+item+"' data-toggle='tooltip' title='Istanze' data='"+JSON.stringify(response[item].istanze)+"' width='20' class='cursorPointer' src='"+req.pathAssImg+"open.png'>");
					...
					$openIstance.off().on("click", function(){
						...
						// 	$("#closeModalIstanze").on("click", function(){
						// 		$('#modalLIstaIstanze').modal('hide');
						// 	});
						// });

						showLoading({domContainer:$("modalLIstaIstanze")});
						$("#titleListIstanzeModal").html($(this).attr("nomeCps"));
						var dataIstanze = JSON.parse($(this).attr("data"));
						$containerListIstanze.clear();
						...
						$("#modalLIstaIstanze").modal({
							...
							for (istanza in dataIstanze){
								for (item in dataIstanze[istanza]){
									if (item.split("-").length>1){
										$tr = $("<tr>"); 
										$tr.append($("<td>").append(getFormatDate(dataIstanze[istanza][item])));
										$tr.append($("<td>").append(istanza));
										$tr.append($("<td>").append(item.split("-")[0]));
										$tr.append($("<td>").append(dataIstanze[istanza][item.split("-")[0]]));
										$containerListIstanze.row.add($tr);
									}
								}
							}
							$containerListIstanze.draw();
							hideLoading({domContainer:$("modalLIstaIstanze")});
						});
					...
					var 
						$semaforoContainer = $("<span class='marginRight5px'>"),
						$containerIstanze = $("<span class='marginRight5px'>"),
						$warningContainer = $("<span class='marginRight5px'>"),
						$imgWarning = $("<img width='23' data-toggle='tooltip' src='"+req.pathAssImg+"warning.png'>");
						$infoContainer = $("<span>"),
						$semaforo = $("<div class='semaforo'></div>"),
						$crossStato = $("<span class='marginRight5px'>"),
						stato = response[item]["statoIstanze"];

					$semaforo.attr("data-toggle", "tooltip");
					$semaforoContainer.append($semaforo.addClass((stato=="G"?"bgGreen":(stato=="R"?"bgRed":"bgYellow"))));
					$semaforoContainer.append($semaforo.attr("title", (stato=="G"?"Tutte le istanze attive":(stato=="R"?"Tutte le istanze non attive":"Qualche istanza non attiva"))));

					// if (response[item].FS === 0){
					// 	$crossStato.html("<img width='16' data-toggle='tooltip' title='Nessun file system' src='"+req.pathAssImg+"cross.png'>");
					// }else{
					// 	$crossStato.html("<img width='16' data-toggle='tooltip' title='File system' src='"+req.pathAssImg+"check.png'>");
					// }

					if (response[item].UPD === 0){
						$imgWarning
							.attr("title", "Nessun warning")
							.attr("style","filter: grayscale(100%);")
						// $warningContainer.html("<img width='23' data-toggle='tooltip' title='Nessun warning' style='filter: grayscale(100%);' src='"+req.pathAssImg+"warning.png'>");
					}else{
						$imgWarning
							.attr("title", "Warning")
							.attr("cps", item)
							.addClass("cursorPointer")
						// $warningContainer.html("<img width='23' class='cursorPointer' cps='"+item+"' data-toggle='tooltip' title='Warning' src='"+req.pathAssImg+"warning.png'>");
						$imgWarning.on("click", function(){
							getLogCps({
								cps:$(this).attr("cps")
							})
						})
					}
					$warningContainer.append($imgWarning)
					$containerIstanze.append($openIstance).append($istanzaContainer);
					$infoContainer.addClass("infoContainer").append($crossStato).append($semaforoContainer).append($containerIstanze).append($warningContainer);
					$tr.append($("<td class='text-center; display: inherit'>").append($infoContainer));
	...
	function downloadFile(options){
		...
		downloadFilePromise.success(function(response){
            ...
            // }
			window.open(req.urlt+response.filename);
			hideLoading({domContainer:$("body")});
		});
	...
	function getLogCps(options){
		var opt = options || {};
		var getLogCpsPromise = http.ajaxCall({
			method	: "GET",
			url		: req.getLogCps+opt.cps,
			headers	: {
				'Authorization': localStorage.getItem("credential"),
			},
			data : {},
		...
		showLoading({domContainer:$("widgetStatoCPS")});
		getLogCpsPromise.success(function(response){
			if (response["RC"] == "0" || response["RC"]==0){
				...
				$("#modalCpsLog").modal({
					...
					$("#titleCPSModalLog").html(opt.cps);
					var logs = response.log,
						$listRowLog = $("#listLogCpsMeggage");
					$listRowLog.empty();
					for( var index in logs){
						var $li = $("<dd>").html(logs[index]).addClass("ddRowLog");
						$listRowLog.append($li);
					}
					$("#closeModalCpsLog").on("click", function(){
						$listRowLog.empty();
						$('#modalCpsLog').modal('hide');
					});
				});
			}else{
				getAlert({
		...
		getLogCpsPromise.error(function(response){
			getAlert({
				msg:"Errore nel caricamento del programma "+opt.modelName,
				type:"error"
			}).show();
		...
		renderTableListProgrammiPromise.success(function(response){
			if (response["RC"] == "0" || response["RC"] == 0){
				...
				for (elem in response){
					...
					$tr = $("<tr>");
					var $divOper = $("<div></div>");
					var $delFile = $("<img nomeFile='PRGSTORE"+"/"+fileProgram[0]+"' class='cursorPointer' width='20' src='"+req.pathAssImg+"trash.png'>").on("click", function(){
					...
					$tr.append($("<td>").html(fileProgram[4]).addClass("textAlignRight"));
					$tr.append($("<td>").html(fileProgram[5]));
					$tr.append($("<td>").html(fileProgram[7]));
		...
		renderTableListFilestorePromise.success(function(response){
			if (response["RC"] == "0" || response["RC"] == 0){
				...
				for (elem in response){
					...
					$tr = $("<tr>");
					var $divOper = $("<div></div>");
					var $delFile = $("<img nomeFile='FILESTORE/"+fileProgram[0]+"' class='cursorPointer' width='20' src='"+req.pathAssImg+"trash.png'>").on("click", function(){
						$("#nomeFileDelete").html($(this).attr("nomeFile"));
					...
					$tr.append($("<td>").html(fileProgram[4]).addClass("textAlignRight"));
					$tr.append($("<td>").html(fileProgram[5]));
					// $tr.append($("<td>").html(fileProgram[7]));//commento
		...
		renderTableListBarcodePromise.success(function(response){
			if (response["RC"] == "0" || response["RC"]==0){
				...
				for (elem in response){
					...
					$tr = $("<tr>");
					var $divOper = $("<div></div>");
					var $delFile = $("<img nomeFile='BARCODE"+"/"+fileProgram[0]+"' class='cursorPointer' width='20' src='"+req.pathAssImg+"trash.png'>").on("click", function(){
						$("#nomeFileDelete").html($(this).attr("nomeFile"));
	...
	function getListFileCPS(options){
		...
		getListFileCPSPromise.success(function(response){
			$("#modalFIleCPS").modal({
				...
				$("#closeModalFileCPSLog").off().on("click", function(){
					$("#modalFIleCPS").modal("hide");
				});
				$("#closeModalFIleCPS").off().on("click", function(){
					$("#modalFIleCPS").modal("hide");
				...
				if (response["RC"] == "0" || response["RC"] == 0){
					...
					for (elem in response){
						fileProgram = response[elem];
						$tr = $("<tr>");
						var $delFile = $("<img nomeFile='"+opt.cps+(opt.dir!= undefined?"/"+opt.dir:"")+"/"+fileProgram[0]+"' class='cursorPointer' width='20' src='"+req.pathAssImg+"trash.png'>").on("click", function(){
							$("#nomeFileDelete").html($(this).attr("nomeFile"));
							$('#modalFIleCPS').modal('hide');
						...
						var $divOper = $("<div></div>");
						if (fileProgram[6]==="dir"){
							$divOper.append($("<img width='20' path='"+opt.cps+(opt.dir!= undefined?"/"+opt.dir:"")+"/"+fileProgram[0]+"' class='iconDisabled' src='"+req.pathAssImg+"trash.png'>"));
	...
	function attemptMask(options){
		return {
			show:showLoading(options),
			hide:hideLoading(options)
		}
	}
